...
一个本地运行的 Vite + React + TypeScript 网页项目，用于分析《崩坏：星穹铁道》成就数据。内置数据已更新至 4.3 版本，共 1811 条成就记录。
...
所有修改会自动写入浏览器 localStorage。更换浏览器、清理站点数据或使用隐私模式可能会导致本地记录不可用。顶部重置按钮会恢复到内置的 4.3 成就大全数据。
